...
namespace UI.Controllers
{
    public class RiddleController : BaseController
    {
        ...
        private const string str_a_activityname = "【正月十五猜灯谜】";
        private const string str_b_activityname = "【正月十五猜灯谜】发生重定向-现金红包跳转";
        private const string str_c_activityname = "【正月十五猜灯谜】发生重定向-实物红包跳转";
        private const string str_d_activityname = "【正月十五猜灯谜】-实物已领取";
        private const string str_e_activityname = "【正月十五猜灯谜】-取奖品awardsModel.Class == \"\"";
        private const string str_f_activityname = "【正月十五猜灯谜】-取奖品awardsModel.Class == null";
        ...
        /// <returns></returns>
        public ActionResult Activity(string activityId, string flag, string guid, string place)
        {
            #region +测试
            ...
            try
            {
                ...
                string url = string.Format("{0}?activityId={1}&place={2}", wx_url, activityId, place);
        ...
        public ActionResult WXActivity(RequestModel request)
        {
            ...
            try
            {
                ...
                if (wxUser == null)
                {
                    ...
                    string url = string.Format("{0}?activityId={1}&place={2}", enter_url, request.activityId,request.place);
                    return empty;
                }
                ...
                if (request.place == "zj")
                {
                    if (isAttendToday(request.activityId, wxUser.Openid, 6))
                    {
                        //判断奖品是否已领
